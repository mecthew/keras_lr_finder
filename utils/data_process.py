...
@timeit
def extract_melspectrogram(data, sr=16000, n_mel=128, use_power_db=True):
    results = []
    for d in data:
        r = librosa.feature.melspectrogram(d, sr=sr, n_mels=n_mel)
        if use_power_db:
...
@timeit
def extract_cqt_parallel(data, sr=16000, n_fft=None, hop_length=None, n_chroma=12):
    if n_fft is None:
        n_fft = int(sr*FFT_DURATION)
    if hop_length is None:
        hop_length = int(sr*HOP_DURATION)

    extract = partial(extract_for_one_sample, extract=librosa.feature.chroma_cqt,
                      sr=sr, hop_length=hop_length, n_chroma=n_chroma)
    results = extract_parallel(data, extract)
